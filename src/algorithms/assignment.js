...
export const assignmentAlgorithm = ({ algorithm, numRows, numColumns, matrix }) => {
    ...
    while (true) {
        ...
        for (let i = 0; i < numRows; i++) {
            ...
            colZeros = costAux[0].map((_, i) => costAux.map(row => row[i]).filter(val => val === 0).length);
            ...
            if (Math.max(...rowZeros) > Math.max(...colZeros)) {
                ...
                costAux[rowZeros.indexOf(Math.max(...rowZeros))].forEach((val, i) => {
    ...
    const assignmentMatrix = Array.from({ length: numRows }, () => Array.from({ length: numColumns }, () => null));
    ...
    for (let i = 0; i < numRows; i++) {
        ...
        let colIndex = 0;
        const colZeros = cost[0].map((_, i) => cost.map(row => row[i]).filter(val => val === 0).length);
        for (let j = 0; j < numColumns; j++) {
            if (cost[i][j] === 0 && colZeros[j] < minZeros && occupiedColumns[j] === null) {
                colIndex = j;
                minZeros = colZeros[j];
            }
        }
