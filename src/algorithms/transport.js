...
export const transportAlgorithm = ({
  ...
  while (!isOptimal) {
    ...
    if (!isOptimal) {
      ...
      copyAllocationMatrix = allocationMatrix.map((row) =>
        row.map((cell) => cell)
      );
      if (flag) {
        break;
...
function validation(numRows, numColumns, matrix) {
  ...
  if (sumSupply !== sumDemand)
    throw new Error(
      "La suma de los elementos del vector de disponibilidad debe ser igual a la suma de los elementos del vector de demanda"
    );
}
